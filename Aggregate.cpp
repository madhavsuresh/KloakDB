...
uint8_t * build_aggregate_from_map_int(flatbuffers::FlatBufferBuilder &b, std::map<int, int> agg, const FieldDesc *fd) {
    //first create a two
    std::vector<flatbuffers::Offset<FieldDesc>> field_vector_desc;
    auto col_name1 = b.CreateString("count");
    auto fd1 = CreateFieldDesc(b, FieldType_INT, col_name1,100);
...
template <class T>
uint8_t * aggregate_count_varchar(flatbuffers::FlatBufferBuilder &b, const GroupByDef *gb_def, const Table *t) {
    ...
    auto ret = build_aggregate_from_map_str(b, agg, t->schema()->fielddescs()->LookupByKey(gb_def->colno()));
    return ret;

}

template <class T>
uint8_t * aggregate_count_int(flatbuffers::FlatBufferBuilder &b, const GroupByDef *gb_def, const Table *t) {
    auto tuples = t->tuples();
    auto col = gb_def->colno();
    std::map<T, int> agg;
    for (int i = 0; i < (int) tuples->Length(); i++) {
...
template <class T>
uint8_t * aggregate_count_timestamp(flatbuffers::FlatBufferBuilder &b, const GroupByDef *gb_def, const Table *t) {
    auto tuples = t->tuples();
    auto col = gb_def->colno();
    std::map<T, int> agg;
    for (int i = 0; i < (int) tuples->Length(); i++) {
...
template <class T>
uint8_t * aggregate_count(flatbuffers::FlatBufferBuilder &b, const GroupByDef *gb_def, const Table *t, FieldType ft) {
    std::map<T, int> agg;
    switch (ft) {
        case FieldType_VARCHAR: {
