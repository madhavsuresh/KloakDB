...
SELECT * INTO vit_ex FROM (SELECT * FROM vitals WHERE pulse IS NOT NULL AND patient_id in (SELECT * from hd_cohort_dist)) t;
SELECT * INTO meds_ex FROM (SELECT * FROM medications WHERE patient_id in (SELECT * from hd_cohort_dist)) t; ---AND medication ILIKE '%aspirin%') t;
