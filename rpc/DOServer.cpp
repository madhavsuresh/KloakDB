...
class DOServerImpl final : public vaultdb::VaultDBOperators::Service {
  virtual Status DBQuery(ServerContext *context, const DBQueryRequest *request,
                         DBQueryResponse *response) {
    table_builder_t *tb =
        table_builder(request->query_string(), request->dbname());
    for (int i = 0; i < tb->table->num_tuples; i++) {
      auto tup = get_tuple(i, tb->table);
      fprintf(stderr, "[%d, %d]", tup->field_list[0].f.int_field.val,
              tup->field_list[1].f.int_field.val);
    }
    return Status::OK;
  }

  virtual Status GetTableStream(ServerContext *context,
                                const TableRequest *request,
                                ServerWriter<TableStream> *writer) {
    table_builder_t *tb =
        table_builder("SELECT * FROM rpc_test;", "dbname=test");
    TableStream t;
  ...
  virtual Status NumHosts(::grpc::ServerContext *context,
  ...
  virtual Status GetTable(ServerContext *context,
