...
void exp5_plain(HonestBrokerPrivate *p,  bool sgx) {
  ...
  START_TIMER(repartition);
  auto repart_a = p->RepartitionJustHash(scan_a);
  auto repart_b = p->RepartitionJustHash(scan_b);
  auto repart_c = p->RepartitionJustHash(scan_c);
  auto repart_d = p->RepartitionJustHash(scan_d);
  auto repart_e = p->RepartitionJustHash(scan_e);
  END_AND_LOG_EXP5_STAT_TIMER(repartition, 0);
  LOG(EXEC) << "======End Repartition====";
  auto to_join1 = zip_join_tables(repart_a, repart_b);
  JoinDef jd;
  jd.set_l_col_name("b");
  ...
  START_TIMER(join1);
  auto out1 = p->Join(to_join1, jd, sgx);
  END_AND_LOG_EXP5_STAT_TIMER(join1, 0);
  LOG(EXEC) << "======END Join 1====";
  auto to_join2 = zip_join_tables(repart_c, out1);
  LOG(EXEC) << "======Start Join 2====";
  START_TIMER(join2);
  auto out2 = p->Join(to_join2, jd, sgx);
  END_AND_LOG_EXP5_STAT_TIMER(join2, 0);
  LOG(EXEC) << "======END Join 2====";
  auto to_join3 = zip_join_tables(repart_d, out2);
  LOG(EXEC) << "======Start Join 3====";
  START_TIMER(join3);
