...
void merge_tuple(tuple_t * to_fill, tuple_t * left_tup, tuple_t * right_tup, join_def_t def) {
  ...
  memset(to_fill, '\0', def.project_len*sizeof(field_desc_t) + sizeof(tuple_t));
...
table_t *HashJoin(table_t *left_table, table_t *right_table, join_def_t def) {
  ...
  schema_t schema = build_join_schema(left_table, right_table, def);
    ...
    for (int i = 0; i < right_table->num_tuples; i++) {
        ...
        auto charthing = tup2->field_list[def.r_col].f.fixed_char_field.val;
        auto intthing = tup2->field_list[def.r_col].f.int_field.genval;
