...
TEST_F(sort_test, string_type) {
    ...
    pqxx::result res2;
    res2 = query(query1, dbname);
    query1 = "INSERT INTO sort_test (a,b) VALUES ('hello',6), ('world',3), ('foo',1)";
    res2 = query(query1, dbname);
    query1 = "SELECT * FROM sort_test;";
    table_builder_t *tb = table_builder(query1, dbname);
    sort_t sortex = {.colno = 1,.type = INT,.ascending = true};
...
void large_random_sort(int power_of_two) {
    ...
    dbname = "dbname=test";
    char buf[128];
    sprintf(buf, "create table if not exists test_random_sort as select s, floor(random() * 100 +1)::int from generate_series(1,%d) s;", power_of_two);
...
TEST_F(sort_test, large_random_sort_512) {
    large_random_sort(512);
}

TEST_F(sort_test, large_random_sort_256) {
    large_random_sort(256);
}

TEST_F(sort_test, large_random_sort_600) {
    large_random_sort(600);
}


TEST_F(sort_test, large_random_sort_90000) {
    large_random_sort(90000);
}

TEST_F(sort_test, large_random_sort_500000) {
    large_random_sort(500000);
}

TEST_F(sort_test, large_random_sort_1000000) {
    large_random_sort(1000000);
}

TEST_F(sort_test, large_random_sort_4000000) {
    large_random_sort(4000000);
}
